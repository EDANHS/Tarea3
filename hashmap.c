...
Buckets * createPair( char * key,  void * value) {
    key = eliminarEspacios(key);
    Buckets * new = (Buckets *)malloc(sizeof(Buckets));
    new->key = key;
    new->value = value;
    return new;
}
...
void insertMap(HashMap * map, char * key, void * value) {
    ...
    Buckets *aux = createPair(key,value);
...
void enlarge(HashMap * map) {
    ...
    Buckets **Aux = map->buckets;
    long capacidad = map->capacity;
    map->capacity *= 2;
    map->buckets = (Buckets**)malloc(sizeof(Buckets*) * map->capacity);
    memset(map->buckets,0,map->capacity * sizeof(Buckets*));
    map->size = 0;
...
HashMap * createMap(long capacity) {
    ...
    Buckets **Par = (Buckets**)calloc(sizeof(Buckets*), capacity);
    Mapa->buckets = Par;
...
Buckets * searchMap(HashMap * map,  char * key) {
    key = eliminarEspacios(key);
    long posicion = hash(key,map->capacity);
...
//hacer m�s estable la funcion searchMap
